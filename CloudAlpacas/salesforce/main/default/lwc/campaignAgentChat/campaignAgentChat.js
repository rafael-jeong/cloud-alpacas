...
import sendMessageApex from '@salesforce/apex/CampaignAgentChatController.sendMessage';
import endConversationApex from '@salesforce/apex/CampaignAgentChatController.endConversation';
import USER_ID from '@salesforce/user/Id';
import MASCOT_URL from '@salesforce/resourceUrl/CA_Alpaca_Mascot';

const STORE_PREFIX = 'caCampAgentHist';
const MAX_CONVERSATIONS = 25;
const MAX_MESSAGES_PER_CONVERSATION = 60;
const TITLE_MAX = 40;

/**
 * Compact Campaign Agent composer (Campaign record page).
 * Owns all transport + browser-side conversation history; delegates the wide
 * list / detail experience to c-campaign-agent-chat-modal.
 *
 * Cloned from opportunityAgentChat — see that component for the original
 * pattern. Differences: no cross-component message-channel signal (no
 * "Stage Guidance"-style trigger exists on Campaign yet), and the current
 * Campaign's Name (not just its Id) is sent to the controller because
 * Sponsorship_Campaign_Agent resolves records by name, not by Record Id.
 */
export default class CampaignAgentChat extends LightningElement {
    @api recordId;
    ...
    wiredCampaign({ data }) {
        if (data) {
            this.campaignName = getFieldValue(data, NAME_FIELD);
        }
    }
    ...
    // --- persistence (localStorage, per user + Campaign) ----
