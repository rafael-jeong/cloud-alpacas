...
**✅ 2026-08-31 최종 갱신 — 실제 구현 충돌 발견 및 해소**: 승우가 같은 날 만든 `Sponsorship_Proposal_Assistant`(PermSet `CA_Opportunity_Agent_Access` 196줄 + Apex 3개, 커밋 `f762840f`)는 `origin/main`에 merge된 적이 없었고, 그 사이 같은 이름의 컴포넌트를 Opportunity Agent 통합 담당자(Eunyeong Doh)가 독립적으로 다시 만들어(PermSet은 승우보다 2.5시간 먼저, Apex 3개는 이틀 뒤) 그 버전이 `origin/main`과 Production Org에 Active로 반영되어 있었다. 두 사람이 서로 모른 채 같은 것을 각자 만든 것으로 보인다. **승우가 검토 후 "Eunyeong의 구현이 맞다"고 확인 — Eunyeong의 버전을 공식 채택하고, 승우의 `f762840f` 버전은 Superseded 처리한다.** 승우 쪽 문서(`Sponsorship_Proposal_Assistant-AgentSpec.md`, `PROPOSAL_QUOTE_AGENT_TEAM_SHARE.md`)에도 동일하게 Superseded 표기를 추가했다. 상세 경위는 `P2_RESULT_REPORT/Individual Development Handoff — Seungwoo — Cloud Alpacas.md` §4-C-1, §19 참고.
...
## Decision 023 — [P2] Campaign 스코프 동결(완성선 5개 항목) — RecordType 4종 확정, Deliverable 데이터 정합성 확보, Renewal 성과 요약 Flow 완성, Campaign Hierarchy 설계 완료 선언

**상태**: ✅ 확정(2026-08-27, 5개 항목 전부 완료 — 아래 "✅ 최종 갱신" 참고)
**기록일**: 2026-08-27

### 배경

Product/Quote는 "기존 스코프 + 고도화 부분까지 이만하면 됐다"는 판단이 이미 섰지만, 같은 기준으로 볼 때 Campaign은 진행도가 그만큼 정리되지 않은 상태였다(승우 판단, 2026-08-27). `B2B_CAMPAIGN_QUOTE_UNDOCUMENTED_IMPLEMENTATION.md` §1~28을 근거로 "기획 대비 구현 상태"를 분석한 결과, Campaign 영역에서 완성선(Definition of Done)에 필요한 항목을 아래 5가지로 좁혔고, 승우가 동의해 바로 실행에 들어갔다.

이 Decision은 Product/Quote/Campaign에 대한 승우의 결정 권한 원칙(파트 내 고도화는 팀 결정 없이 즉시 실행 가능, 단 다른 팀원 파트의 필드 변경·데이터 생성을 유발하는 경우에만 사전 협의, 파트 경계를 넘나든 과정은 항상 사후 보고)에 따라 진행됐다 — 아래 4번 항목이 그 보고에 해당한다.

### 결정

1. **Campaign RecordType을 현재 4종(Fan_Campaign, Sponsorship_Prospecting, Sponsorship_Collaboration, Sponsorship_Renewal)에서 동결한다.** 이 라인을 넘는 추가 RecordType이나 Campaign 관련 신규 Custom Object 생성은 하지 않는다(Decision 006 "필요한 만큼만" 원칙 재확인). §15(2026-08-26, 2종→4종 확장)가 이 확정의 근거 기록이다.
2. **`Campaign_Deliverable__c`는 계속 유지하며(존치 여부 재논의 종료), 데이터 정합성을 전체 Collaboration 캠페인으로 확장한다.** d'Alba 2년차 협업 캠페인에만 있던 Deliverable 레코드(4건)를 기준으로, 나머지 실제 Collaboration 캠페인 3개(d'Alba 1년차, 그린빈 커피, 루나 뷰티) 각각에 4건씩 총 12건을 신규 생성해 커버리지를 100%로 맞췄다(2026-08-27 완료).
3. **`Renewal_Campaign_Performance_Summary` Flow를 완성해 Active로 전환한다.** 작업 중 §25에 기록되지 않았던 문제 2건을 추가로 발견해 함께 해결했다 — (a) `Get_Collab_Campaigns` 조회의 filter 1개가 `null__NotFound`라는 깨진 필드 참조로 저장되어 있어 Flow가 실행될 때마다 오류가 나게 되어 있던 것을 `RecordTypeId` 조건으로 교체, (b) Flow가 참조하는 Roll-Up Summary 필드 `Total_Deliverable_Weight__c`/`Completed_Deliverable_Weight__c`가 애초에 생성된 적이 없었던 것을 신규 생성. 그린빈·루나처럼 협업 사이클이 1회뿐이라 Hierarchy가 2단 구조(Collaboration이 Renewal의 직속 Parent)인 경우도 형제 캠페인을 정상적으로 찾도록 조회 조건을 함께 보강했다.
   Flow는 수정된 버전으로 **Active 전환에 성공했고, 실제 d'Alba 2027 시즌 스폰서십 갱신 제안 캠페인(701bm00002j7mEfAAI)에 저장해 `Performance_Summary__c`에 실제 값이 정상적으로 채워짐을 최종 확인**했다(아래 "✅ 최종 갱신" 참고).
4. **Campaign Hierarchy는 설계 완료(design-complete)로 선언한다.** 스폰서 5곳 전체가 Hierarchy로 연결되어 있고 Rollup(`HierarchyExpectedRevenue` 등)도 표준 기능으로 정상 동작함을 API로 확인했다(§17). 다만 재확인 당시 전체 Opportunity 103건 중 `Opportunity.CampaignId`(Primary Campaign Source)가 채워진 건 7건뿐이었다(§17 갱신) — **이건 Campaign 설계의 미완성이 아니라, Opportunity 생성 프로세스에 "Primary Campaign Source 입력"이 정착되지 않은 채택/프로세스 문제이며, Opportunity 파트(은영) 영역이다.**
   **✅ 2026-08-27 추가 조치**: 확인 과정에서 Aaron Choi가 8/25 별도로 생성한 대규모 B2B 파트너십 Opportunity 세트(104건, "~~파트너십 제안"/"~~시즌 공식 파트너십 계약" 명명 패턴)를 발견했다 — 각 회사마다 Campaign이 아예 없어 Primary Campaign Source를 채우고 싶어도 채울 대상 자체가 없는 상태였다. 이건 "필드 입력을 안 한 프로세스 문제"가 아니라 "연결 대상 Campaign 레코드 자체가 없는" 선행 공백이라 판단해, Campaign 파트에서 104개 회사 전부에 Campaign을 신규 생성(Stage 기준 RecordType 매핑: Closed Won→Collaboration 75개, 그 외 진행중/Closed Lost→Prospecting 29개)하고 Opportunity.CampaignId까지 연결했다. Collaboration 75개에는 기존 5개 헤어 스폰서와 동일한 수준으로 Campaign_Deliverable__c 4건씩(총 300건)도 생성했다. 이 작업은 Campaign 자체 신규 레코드 생성(Campaign 파트 소관)과 Opportunity.CampaignId 연결(팀원 파트 데이터를 Campaign 고도화 목적으로 연결 — 승우의 결정 권한 원칙상 허용 범위)로 구성되며, 사후 보고 대상이라 여기 기록한다.
   결과: 전체 Opportunity 110건 중 테스트/불완전 데이터 3건(Company Ever, 라온푸드-, Account 없는 "A사 2026 Sponsorship")을 제외한 **107건 전부 Campaign 연결 완료** — Hierarchy Rollup의 실사용 공백이 사실상 해소됐다. 은영님께는 이제 "입력해달라"가 아니라 "이미 다 연결해뒀다"는 결과 공유 + 앞으로 신규 Opportunity 생성 시에도 Campaign 연결을 프로세스에 넣어달라는 요청으로 내용이 바뀐다(아직 미전달, 다음 액션).
5. **위 1~4가 끝났으므로 Campaign 스코프를 공식 동결한다** — Product/Quote와 같은 수준의 "이만하면 완성" 선언. Campaign RecordType/Object 구조, Deliverable 데이터, Renewal 성과 요약 Flow, Hierarchy 설계 전부 실사용 가능한 수준으로 완료됨.

### ✅ 2026-08-27 최종 갱신 — 3항 완료, Decision 확정

3항이 막혀있던 원인은 이 Org 고유의 플랫폼 특이사항이었다: **Metadata API(`sf project deploy`)로 배포한 CustomField가 describe/SOQL/REST/Apex 계층에 반영되지 않는 현상**(이번 세션에서만 최소 7개 필드, `Performance_Summary__c`는 어제 세션의 delete+재생성 이후로 24시간 넘게 지속)이 근본 원인이었다. **Setup UI의 "New Custom Field" 마법사로 수동 생성하면 즉시 정상 작동**한다는 해결책을 발견해(PricebookEntry 작업 중 우연히 발견, 별도 Decision 대상 아님 — 기술적 워크어라운드), Campaign의 4개 필드(`Progress_Percent__c`/`Performance_Summary__c`/`Total_Deliverable_Weight__c`/`Completed_Deliverable_Weight__c`)에도 동일하게 적용했다.

절차: Flow에서 3개 필드 참조 임시 제거 → Draft 재배포 → Active 전환(구버전 자동 Obsolete) → Tooling API로 구버전 삭제 → 참조 사라진 필드 삭제 → 승우가 Setup UI에서 4개 필드 수동 재생성(라벨 한국어로 확정: 성과 요약/전체 이행 항목 가중치/완료된 이행 항목 가중치) → 즉시 정상 작동 확인 → Flow에 참조 복원 → Active 재배포 → 실제 캠페인 저장 테스트.

**최종 결과** (d'Alba 2027 시즌 갱신 제안 캠페인, `Performance_Summary__c` 실제 값):
> [스폰서십 성과 요약 - Platinum 파트너 갱신 참고자료] ■ 팬 도달 및 반응 — 총 노출 2명, 반응율 0% ■ 약속 이행 신뢰도 — 40% 완료. 이 성과를 바탕으로 갱신 또는 Diamond 승급을 제안드립니다.

이행률 40%는 d'Alba 1·2년차 캠페인의 실제 Deliverable 데이터(총 200, 완료 80)와 정확히 일치하고 티어(Platinum)도 실제 Opportunity와 일치 — 형제 캠페인 순회부터 요약문 생성까지 전 과정이 실제 데이터로 end-to-end 검증됐다.

**따라서 5개 항목 전부 완료되어 이 Decision을 확정한다.** 상세 경위는 `B2B_CAMPAIGN_QUOTE_UNDOCUMENTED_IMPLEMENTATION.md` §25 최종 종결 절 참고.

### 이 Decision이 바꾸지 않는 것

- Decision 015(Campaign_Deliverable__c·Path 등 B2B 워크플로 도입), 018-시리즈(Object 결정), 019(Sponsorship Sales/Pipeline 중심) — 전부 유지.
- Campaign Member Status 5단계, 표준 재무 필드 노출, Report/Dashboard 등 이미 완료로 표시된 §1~24 항목은 재논의하지 않는다.

### 이유

Product/Quote가 "완성"으로 선언된 기준(기존 스코프 + 고도화, 실사용 가능한 수준)을 Campaign에도 동일하게 적용하려면, "설계는 됐지만 실행이 안 된 것"(Deliverable 데이터 공백, Flow 미완성)과 "설계 자체가 이 파트 밖의 문제인 것"(Hierarchy의 Opportunity 채택 공백)을 구분해야 한다. 전자는 승우 파트 내에서 바로 끝낼 수 있는 일이었고, 후자는 아무리 Campaign 쪽에서 손봐도 해결되지 않는(Opportunity 프로세스 문제) 항목이라 완성선 판정 기준에서 분리했다.

### 영향

- `P2_RESULT_REPORT/B2B_CAMPAIGN_QUOTE_UNDOCUMENTED_IMPLEMENTATION.md` §25, §27이 이 Decision을 근거로 갱신됨.
- 은영님에게 Opportunity.CampaignId 입력 프로세스 공유가 필요(아직 미전달 — 별도 액션 필요, 이 Decision의 4번 항목 참고).

### TBD

- Opportunity.CampaignId 프로세스 정착은 Campaign Decision 범위가 아니라 은영님/Opportunity 파트의 판단 사항(현재 103건 중 7건으로 재확인, §17 갱신) — 은영님께 아직 미전달, 별도 공유 필요
- 이 Org의 "Metadata API 배포 필드가 반영 안 되는" 플랫폼 특이사항은 Campaign 밖에서도 재발할 수 있음 — 다른 팀원도 새 CustomField 배포 후 안 보이면 이 Decision과 `B2B_CAMPAIGN_QUOTE_UNDOCUMENTED_IMPLEMENTATION.md` §25를 참고해 Setup UI 수동 재생성으로 대응할 것
